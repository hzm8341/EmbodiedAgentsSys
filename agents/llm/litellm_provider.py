...
Ported from RoboClaw/roboclaw/providers/litellm_provider.py.
Changes:
  - loguru → stdlib logging
  - import path: roboclaw.providers → agents.llm
"""
...
from agents.llm.registry import find_by_model, find_gateway
...
class LiteLLMProvider(LLMProvider):
    ...
    Supports OpenRouter, Anthropic, OpenAI, Gemini, DashScope, and many other
    providers through a unified interface. Provider-specific logic is driven by
    the registry (agents/llm/registry.py) — no if-elif chains needed here.
    """
    ...
    def _parse_response(self, response: Any) -> LLMResponse:
        ...
        if len(response.choices) > 1:
            logger.debug(
                "LiteLLM response has %d choices, merged %d tool_calls",
