...
class TaskPlanner:
    ...
    def _build_planning_prompt(self, task_description: str, context: ExecutionContext) -> str:
        """构建规划提示词"""
        ...
        try:
            ...
            for i, step_data in enumerate(step_list):
                step = TaskStep(
                    ...
                    skill_name=self._normalize_skill_name(step_data.get("skill_name", "")),
...
class TaskExecutor:
    ...
    def __init__(
        ...
        on_step_complete: Optional[Callable[[TaskStep, ExecutionResult], Awaitable]] = None,
    ):
        """
        初始化执行器
    ...
    async def execute_plan(
        ...
        for step in executable_steps:
            ...
            if result.status == TaskStatus.FAILED and stop_on_error:
                ...
                print(f"[TaskExecutor] Step {step.step_id} failed, stopping plan execution")
                break
